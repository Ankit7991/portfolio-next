...
export function useRandomPositions(
	...
	useEffect(() => {
		...
		console.log(`%%`,freeSpaces, elements, Math.floor(Math.random() * (freeSpaces.length - elements.length) - 1));
		// const randomLocations = (getRandomElements(freeSpaces, elements.length)).map((el, i: number) => {
